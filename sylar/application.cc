...
int Application::run_fiber() {
    ...
    for (auto& i : http_confs) {
        ...
        if (!i.io_worker.empty()) {
            io_worker = sylar::WorkerMgr::GetInstance()
        ...
        if (i.type == "http") {
            ...
            server.reset(new sylar::http::WSServer(process_worker, io_worker,
                                                   accept_worker));
        } else if (i.type == "rock") {
            server.reset(new sylar::RockServer("rock", process_worker,
                                               io_worker, accept_worker));
        } else if (i.type == "nameserver") {
            server.reset(new sylar::RockServer("nameserver", process_worker,
                                               io_worker, accept_worker));
        } else {
            SYLAR_LOG_ERROR(g_logger)
