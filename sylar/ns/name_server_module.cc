...
#define XX(info, attr)                                                   \
    if (!info.has_##attr()) {                                            \
        ...
        if (info.cmds_size() == 0) {
            SYLAR_LOG_ERROR(g_logger)
                << "invalid register request from: "
                << stream->getRemoteAddressString() << " cmds is null";
            return false;
        }
        auto& node = info.node();
        XX(node, ip);
...
void NameServerModule::set(sylar::RockStream::ptr rs,
                           NSClientInfo::ptr new_value) {
    if (!rs->isConnected()) {
        new_value = nullptr;
    }
...
std::set<sylar::RockStream::ptr> NameServerModule::getStreams(
    const std::string& domain) {
    sylar::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<sylar::RockStream::ptr>()
                                          : it->second;
}
...
void NameServerModule::setQueryDomain(sylar::RockStream::ptr rs,
