...
ZKClient::ZKClient() : m_handle(nullptr), m_recvTimeout(0) {}
...
void ZKClient::OnWatcher(zhandle_t* zh,
                         int type,
                         int stat,
                         const char* path,
                         void* watcherCtx) {
...
bool ZKClient::reconnect() {
    ...
    m_handle = zookeeper_init2(m_hosts.c_str(), &ZKClient::OnWatcher,
                               m_recvTimeout, nullptr, this, 0, m_logCb);
...
bool ZKClient::init(const std::string& hosts,
    ...
    m_logCb = lcb;
    m_handle = zookeeper_init2(hosts.c_str(), &ZKClient::OnWatcher,
                               m_recvTimeout, nullptr, this, 0, lcb);
    return m_handle != nullptr;
}
...
int32_t ZKClient::create(const std::string& path,
                         ...
                         int flags) {
    return zoo_create(m_handle, path.c_str(), val.c_str(), val.size(), acl,
                      flags, &new_path[0], new_path.size());
}
...
int32_t ZKClient::set(const std::string& path,
                      ...
                      int version,
                      Stat* stat) {
    return zoo_set2(m_handle, path.c_str(), val.c_str(), val.size(), version,
                    stat);
}
...
int32_t ZKClient::getChildren(const std::string& path,
    ...
    int32_t rt =
        zoo_get_children2(m_handle, path.c_str(), watch, &strings, stat);
    if (rt == ZOK) {
