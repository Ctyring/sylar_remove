...
uint64_t murmur3_hash64(const void* str,
    ...
    return (((uint64_t)murmur3_hash(str, size, seed)) << 32 |
            murmur3_hash(str, size, seed2));
}
uint64_t murmur3_hash64(const char* str,
                        const uint32_t& seed,
                        const uint32_t& seed2) {
    return (((uint64_t)murmur3_hash(str, seed)) << 32 |
...
std::string replace(const std::string& str1,
                    char find,
