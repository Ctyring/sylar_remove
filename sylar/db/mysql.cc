...
static sylar::ConfigVar<
    ...
    g_mysql_dbs = sylar::Config::Lookup(
        "mysql.dbs",
...
static MYSQL* mysql_init(std::map<std::string, std::string>& params,
    ...
    if (mysql_real_connect(mysql, host.c_str(), user.c_str(), passwd.c_str(),
        ...
        SYLAR_LOG_ERROR(g_logger)
            << "mysql_real_connect(" << host << ", " << port << ", " << dbname
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    ...
    if (::mysql_query(mysql, sql)) {
        SYLAR_LOG_ERROR(g_logger)
            << "mysql_query(" << sql << ") error:" << mysql_error(mysql);
        return nullptr;
    }
    ...
    if (res == nullptr) {
        SYLAR_LOG_ERROR(g_logger)
            << "mysql_store_result() error:" << mysql_error(mysql);
    }
    return res;
}
...
MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string& stmt) {
    ...
    if (mysql_stmt_prepare(st, stmt.c_str(), stmt.size())) {
        SYLAR_LOG_ERROR(g_logger)
            << "stmt=" << stmt << " errno=" << mysql_stmt_errno(st)
...
MySQLStmtRes::ptr MySQLStmtRes::Create(std::shared_ptr<MySQLStmt> stmt) {
    ...
    if (!res) {
        return MySQLStmtRes::ptr(
            new MySQLStmtRes(stmt, stmt->getErrno(), stmt->getErrStr()));
    }
...
MySQLStmtRes::MySQLStmtRes(std::shared_ptr<MySQLStmt> stmt,
                           int eno,
...
MySQLTransaction::ptr MySQLTransaction::Create(MySQL::ptr mysql,
                                               bool auto_commit) {
...
int MySQLTransaction::execute(const char* format, va_list ap) {
    if (m_isFinished) {
        SYLAR_LOG_ERROR(g_logger)
            << "transaction is finished, format=" << format;
        return -1;
    }
...
MySQL::ptr MySQLManager::get(const std::string& name) {
    ...
    if (it != m_conns.end()) {
        if (!it->second.empty()) {
            ...
            if (!rt->isNeedCheck()) {
                ...
                return MySQL::ptr(rt, std::bind(&MySQLManager::freeMySQL, this,
    ...
    if (rt->connect()) {
        rt->m_lastUsedTime = time(0);
        return MySQL::ptr(rt, std::bind(&MySQLManager::freeMySQL, this, name,
                                        std::placeholders::_1));
    } else {
        delete rt;
        return nullptr;
...
void MySQLManager::registerMySQL(
...
int MySQLManager::execute(const std::string& name,
                          const char* format,
                          va_list ap) {
    auto conn = get(name);
    if (!conn) {
...
ISQLData::ptr MySQLManager::query(const std::string& name,
                                  const char* format,
                                  ...) {
    va_list ap;
    va_start(ap, format);
...
ISQLData::ptr MySQLManager::query(const std::string& name,
    ...
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger)
            << "MySQLManager::query, get(" << name << ") fail, sql=" << sql;
        return nullptr;
    }
...
MySQLTransaction::ptr MySQLManager::openTransaction(const std::string& name,
    ...
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger)
            << "MySQLManager::openTransaction, get(" << name << ") fail";
        return nullptr;
    }
...
ISQLData::ptr MySQLUtil::Query(const std::string& name,
                               const char* format,
                               ...) {
    va_list ap;
    va_start(ap, format);
...
ISQLData::ptr MySQLUtil::TryQuery(const std::string& name,
                                  uint32_t count,
                                  const char* format,
                                  ...) {
...
int MySQLUtil::Execute(const std::string& name,
                       const char* format,
                       va_list ap) {
    auto m = MySQLMgr::GetInstance()->get(name);
    if (!m) {
...
int MySQLUtil::TryExecute(const std::string& name,
                          uint32_t count,
                          const char* format,
                          ...) {
    int rpy = 0;
