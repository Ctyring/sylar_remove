...
static sylar::ConfigVar<
    std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
    sylar::Config::Lookup(
        "redis.config",
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if (c) {
        if (m_cmdTimeout.tv_sec || m_cmdTimeout.tv_usec) {
            setTimeout(m_cmdTimeout.tv_sec * 1000 +
                       m_cmdTimeout.tv_usec / 1000);
        }
        ...
        if (!m_passwd.empty()) {
            ...
            if (!r) {
                SYLAR_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":"
                                          << m_port << ", " << m_name << ")";
                return false;
            }
            if (r->type != REDIS_REPLY_STATUS) {
                SYLAR_LOG_ERROR(g_logger)
                    << "auth reply type error:" << r->type << "(" << m_host
                    << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if (!r->str) {
                SYLAR_LOG_ERROR(g_logger)
                    << "auth reply str error: NULL(" << m_host << ":" << m_port
                    << ", " << m_name << ")";
                return false;
            }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
    ...
    if (!r) {
        if (m_logEnable) {
            SYLAR_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host
                                      << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ...
    if (m_logEnable) {
        SYLAR_LOG_ERROR(g_logger)
            << "redisCommandArgv error: (" << m_host << ":" << m_port << ")("
            << m_name << ")" << r->str;
    }
    return nullptr;
}
...
ReplyPtr Redis::getReply() {
    ...
    if (m_logEnable) {
        SYLAR_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":"
                                  << m_port << ")(" << m_name << ")";
    }
    return nullptr;
}
...
bool RedisCluster::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            auto r = (redisReply*)redisClusterCommand(c, "auth %s",
                                                      m_passwd.c_str());
            if (!r) {
                SYLAR_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":"
                                          << m_port << ", " << m_name << ")";
...
ReplyPtr RedisCluster::cmd(const std::vector<std::string>& argv) {
    ...
    auto r = (redisReply*)redisClusterCommandArgv(m_context.get(), argv.size(),
                                                  &v[0], &l[0]);
    if (!r) {
        if (m_logEnable) {
            SYLAR_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host
...
int RedisCluster::appendCmd(const std::vector<std::string>& argv) {
    ...
    return redisClusterAppendCommandArgv(m_context.get(), argv.size(), &v[0],
                                         &l[0]);
}

FoxRedis::FoxRedis(sylar::FoxThread* thr,
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if (!r) {
        SYLAR_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":"
                                  << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
        SYLAR_LOG_ERROR(g_logger)
            << "auth reply type error:" << r->type << "(" << fr->m_host << ":"
            << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (!r->str) {
        SYLAR_LOG_ERROR(g_logger)
            << "auth reply str error: NULL(" << fr->m_host << ":" << fr->m_port
            << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        SYLAR_LOG_INFO(g_logger)
            << "auth ok: " << r->str << "(" << fr->m_host << ":" << fr->m_port
            << ", " << fr->m_name << ")";
    } else {
        SYLAR_LOG_ERROR(g_logger)
            << "auth error: " << r->str << "(" << fr->m_host << ":"
            << fr->m_port << ", " << fr->m_name << ")";
    }
}
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host
                                 << ":" << c->c.tcp.port << " success";
        ar->m_status = CONNECTED;
        if (!ar->m_passwd.empty()) {
            int rt = redisAsyncCommand(ar->m_context.get(), FoxRedis::OnAuthCb,
                                       ar, "auth %s", ar->m_passwd.c_str());
            if (rt) {
        ...
        SYLAR_LOG_ERROR(g_logger)
            << "FoxRedis::ConnectCb " << c->c.tcp.host << ":" << c->c.tcp.port
            << " fail, error:" << c->errstr;
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    SYLAR_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb " << c->c.tcp.host
...
void FoxRedis::delayDelete(redisAsyncContext* c) {
    ...
    // Res* res = new Res();
    // res->ctx = c;
    // struct event* event = event_new(m_thread->getBase(), -1, EV_TIMEOUT,
    // DelayTimeCb, res); res->event = event;
    //
...
bool FoxRedis::pinit() {
    ...
    if (!ctx) {
        SYLAR_LOG_ERROR(g_logger)
            << "redisAsyncConnect (" << m_host << ":" << m_port << ") null";
        return false;
    }
    if (ctx->err) {
        SYLAR_LOG_ERROR(g_logger)
            << "Error:(" << ctx->err << ")" << ctx->errstr;
        return false;
    }
    ctx->data = this;
    ...
    // m_context.reset(ctx, std::bind(&FoxRedis::delayDelete, this,
    // std::placeholders::_1));
    if (m_event == nullptr) {
        m_event = event_new(m_thread->getBase(), -1, EV_TIMEOUT | EV_PERSIST,
                            TimeCb, this);
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
    ...
    do {
        ...
        char* buf = nullptr;
        int len = redisFormatCommandArgv(&buf, argv.size(), &(args[0]),
                                         &(args_len[0]));
        if (len == -1 || !buf) {
...
void FoxRedis::pcmd(FCtx* fctx) {
    if (m_status == UNCONNECTED) {
        SYLAR_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port
                                 << ") unconnected " << fctx->cmd;
        init();
        if (fctx->fiber) {
    ...
    if (!ctx->cmd.empty()) {
        // redisAsyncCommand(m_context.get(), CmdCb, ctx.get(),
        // ctx->cmd.c_str());
        redisAsyncFormattedCommand(m_context.get(), CmdCb, ctx,
                                   ctx->cmd.c_str(), ctx->cmd.size());
        ...
        //    }
        //    redisAsyncCommandArgv(m_context.get(), CmdCb, ctx.get(),
        //    argv.size(),
        //            &(argv[0]), &(argv_len[0]));
    }
}
...
FoxRedisCluster::FoxRedisCluster(sylar::FoxThread* thr,
                                 const std::map<std::string, std::string>& conf)
    : m_thread(thr), m_status(UNCONNECTED), m_event(nullptr) {
    m_ctxCount = 0;
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c,
                               void* rp,
                               void* priv) {
    ...
    if (!r) {
        SYLAR_LOG_ERROR(g_logger)
            << "auth error:(" << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
    ...
    if (!r->str) {
        SYLAR_LOG_ERROR(g_logger) << "auth reply str error: NULL(" << fr->m_host
                                  << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        SYLAR_LOG_INFO(g_logger) << "auth ok: " << r->str << "(" << fr->m_host
                                 << ", " << fr->m_name << ")";
    } else {
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYLAR_LOG_INFO(g_logger)
            << "FoxRedisCluster::ConnectCb " << c->c.tcp.host << ":"
            << c->c.tcp.port << " success";
        if (!ar->m_passwd.empty()) {
            int rt = redisClusterAsyncCommand(ar->m_context.get(),
                                              FoxRedisCluster::OnAuthCb, ar,
                                              "auth %s", ar->m_passwd.c_str());
            if (rt) {
                SYLAR_LOG_ERROR(g_logger)
                    << "FoxRedisCluster Auth fail: " << rt;
            }
        }
    } else {
        SYLAR_LOG_ERROR(g_logger)
...
void FoxRedisCluster::DisconnectCb(const redisAsyncContext* c, int status) {
    SYLAR_LOG_INFO(g_logger)
        << "FoxRedisCluster::DisconnectCb " << c->c.tcp.host << ":"
        << c->c.tcp.port << " status:" << status;
}

void FoxRedisCluster::CmdCb(redisClusterAsyncContext* ac,
                            void* r,
                            void* privdata) {
    Ctx* ctx = static_cast<Ctx*>(privdata);
    if (ctx->timeout) {
...
bool FoxRedisCluster::pinit() {
    ...
    if (!ctx) {
        SYLAR_LOG_ERROR(g_logger)
            << "redisClusterAsyncConnect (" << m_host << ") null";
        return false;
    }
    if (ctx->err) {
        SYLAR_LOG_ERROR(g_logger)
    ...
    // m_context.reset(ctx, std::bind(&FoxRedisCluster::delayDelete, this,
    // std::placeholders::_1));
    if (m_event == nullptr) {
        m_event = event_new(m_thread->getBase(), -1, EV_TIMEOUT | EV_PERSIST,
                            TimeCb, this);
...
void FoxRedisCluster::pcmd(FCtx* fctx) {
    if (m_status != CONNECTED) {
        SYLAR_LOG_INFO(g_logger)
            << "redis (" << m_host << ") unconnected " << fctx->cmd;
        init();
        if (fctx->fiber) {
            fctx->scheduler->schedule(&fctx->fiber);
        }
    ...
    // ctx->tref = ctx;
    if (!ctx->cmd.empty()) {
        // redisClusterAsyncCommand(m_context.get(), CmdCb, ctx.get(),
        // ctx->cmd.c_str());
        redisClusterAsyncFormattedCommand(m_context.get(), CmdCb, ctx,
                                          &ctx->cmd[0], ctx->cmd.size());
        //} else if(!ctx->parts.empty()) {
        //    std::vector<const char*> argv;
        //    std::vector<size_t> argv_len;
        //    for(auto& i : ctx->parts) {
        ...
        //        argv_len.push_back(i.size());
        //    }
        //    redisClusterAsyncCommandArgv(m_context.get(), CmdCb, ctx.get(),
        //    argv.size(),
        //            &(argv[0]), &(argv_len[0]));
    }
}
...
void FoxRedisCluster::Ctx::cancelEvent() {
    ...
    //            << " - " << cancel_count;
    // if(thread != sylar::FoxThread::GetThis()) {
    //     SYLAR_LOG_INFO(g_logger) << "cancelEvent " <<
    //     sylar::FoxThread::GetThis()
    //                << " - " << thread
    //                << " - " << sylar::IOManager::GetThis()
    //                << " - " << cancel_count;

    //    //SYLAR_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " "
    //    << thread->getId()
    ...
    for (auto& i : m_config) {
        ...
        for (int n = 0; n < pool; ++n) {
            if (type == "redis") {
                ...
                auto conf = i.second;
                auto name = i.first;
                sylar::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        sylar::FoxRedis* rds(new sylar::FoxRedis(
                            sylar::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
                ...
                sylar::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        sylar::FoxRedisCluster* rds(new sylar::FoxRedisCluster(
                            sylar::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
...
ReplyPtr RedisUtil::Cmd(const std::string& name,
