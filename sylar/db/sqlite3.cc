...
ITransaction::ptr SQLite3::openTransaction(bool auto_commit) {
    ITransaction::ptr trans(
        new SQLite3Transaction(shared_from_this(), auto_commit));
    if (trans->begin()) {
        return trans;
    }
    return nullptr;
...
int SQLite3Stmt::prepare(const char* stmt) {
    ...
    return sqlite3_prepare_v2(m_db->getDB(), stmt, strlen(stmt), &m_stmt,
                              nullptr);
}
...
SQLite3Transaction::SQLite3Transaction(SQLite3::ptr db,
                                       bool auto_commit,
                                       Type type)
    ...
    if (m_status == 1) {
        ...
        if (m_status == 1) {
            SYLAR_LOG_ERROR(g_logger)
                << m_db << " auto_commit=" << m_autoCommit << " fail";
        }
    }
}
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
    ...
    if (it != m_conns.end()) {
        if (!it->second.empty()) {
            ...
            lock.unlock();
            return SQLite3::ptr(
                rt, std::bind(&SQLite3Manager::freeSQLite3, this, name,
                              std::placeholders::_1));
        }
    }
    ...
    sqlite3* db;
    if (sqlite3_open_v2(path.c_str(), &db, SQLite3::CREATE | SQLite3::READWRITE,
                        nullptr)) {
        SYLAR_LOG_ERROR(g_logger)
            << "open db name=" << name << " path=" << path << " fail";
        return nullptr;
    }
    ...
    if (!sql.empty()) {
        if (rt->execute(sql)) {
            SYLAR_LOG_ERROR(g_logger)
                << "execute sql=" << sql << " errno=" << rt->getErrno()
                << " errstr=" << rt->getErrStr();
            delete rt;
            return nullptr;
        }
    ...
    return SQLite3::ptr(rt, std::bind(&SQLite3Manager::freeSQLite3, this, name,
                                      std::placeholders::_1));
}

void SQLite3Manager::registerSQLite3(
...
int SQLite3Manager::execute(const std::string& name,
                            const char* format,
                            va_list ap) {
    auto conn = get(name);
    if (!conn) {
...
int SQLite3Manager::execute(const std::string& name, const std::string& sql) {
    ...
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger)
            << "SQLite3Manager::execute, get(" << name << ") fail, sql=" << sql;
        return -1;
    }
    return conn->execute(sql);
}

ISQLData::ptr SQLite3Manager::query(const std::string& name,
                                    const char* format,
                                    ...) {
    va_list ap;
    va_start(ap, format);
...
ISQLData::ptr SQLite3Manager::query(const std::string& name,
    ...
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger)
            << "SQLite3Manager::query, get(" << name << ") fail, sql=" << sql;
        return nullptr;
    }
    return conn->query(sql);
}

SQLite3Transaction::ptr SQLite3Manager::openTransaction(const std::string& name,
                                                        bool auto_commit) {
    auto conn = get(name);
    if (!conn) {
        SYLAR_LOG_ERROR(g_logger)
            << "SQLite3Manager::openTransaction, get(" << name << ") fail";
        return nullptr;
    }
