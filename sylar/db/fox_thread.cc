...
static sylar::ConfigVar<
    ...
    g_thread_info_set = Config::Lookup(
        "fox_thread",
...
void FoxThread::dump(std::ostream& os) {
    ...
    os << "[thread name=" << m_name << " working=" << m_working
...
void FoxThread::thread_cb() {
    // std::cout << "FoxThread(" << m_name << "," << pthread_self() << ")" <<
    // std::endl;
    setThis();
...
bool FoxThread::dispatch(callback cb) {
    ...
    //     std::cout << std::this_thread::get_id() << ":" << m_callbacks.size()
    //     << " " << m_name << std::endl;
    // }
    lock.unlock();
    uint8_t cmd = 1;
...
FoxThreadPool::FoxThreadPool(uint32_t size,
...
void FoxThreadPool::check() {
    do {
        ...
        std::shared_ptr<FoxThread> thr(
            ...
            std::bind(&FoxThreadPool::releaseFoxThread, this,
                      std::placeholders::_1));
...
void FoxThreadManager::dispatch(const std::string& name,
                                uint32_t id,
                                callback cb) {
    IFoxThread::ptr ti = get(name);
    SYLAR_ASSERT(ti);
...
void FoxThreadManager::init() {
    ...
    for (auto i : m) {
        ...
        if (num == 1) {
            ...
            m_threads[i.first] =
                FoxThreadPool::ptr(new FoxThreadPool(num, name, advance));
            SYLAR_LOG_INFO(g_logger)
                << "init thread pool:" << name << " num:" << num
                << " advance:" << advance;
        }
    }
}
