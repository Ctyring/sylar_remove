 ...
 */
inline int32_t binarySearch(const uint16_t* array,
                            int32_t lenarray,
                            uint16_t ikey) {
    int32_t low = 0;
 ...
 */
static inline int32_t advanceUntil(const uint16_t* array,
                                   int32_t pos,
                                   int32_t length,
                                   uint16_t min) {
    int32_t lower = pos + 1;
 ...
 */
static inline int32_t count_less(const uint16_t* array,
                                 int32_t lenarray,
                                 uint16_t ikey) {
    if (lenarray == 0)
        return 0;
 ...
 * Array elements must be unique and sorted.
 */
static inline int32_t count_greater(const uint16_t* array,
                                    int32_t lenarray,
                                    uint16_t ikey) {
    if (lenarray == 0)
        return 0;
...
int32_t intersect_skewed_uint16(const uint16_t* smallarray,
                                size_t size_s,
...
bool intersect_skewed_uint16_nonempty(const uint16_t* smallarray,
                                      size_t size_s,
                                      const uint16_t* largearray,
                                      size_t size_l);
/**
 ...
 */
uint32_t union_vector16(const uint16_t* __restrict__ set_1,
                        uint32_t size_1,
                        const uint16_t* __restrict__ set_2,
                        uint32_t size_2,
 ...
 */
static inline void bitset_set_range(uint64_t* bitmap,
                                    uint32_t start,
                                    uint32_t end) {
    if (start == end)
        return;
 ...
 */
static inline int bitset_lenrange_cardinality(uint64_t* bitmap,
                                              uint32_t start,
 ...
 */
static inline bool bitset_lenrange_empty(uint64_t* bitmap,
                                         uint32_t start,
                                         uint32_t lenminusone) {
    uint32_t firstword = start / 64;
    uint32_t endword = (start + lenminusone) / 64;
    if (firstword == endword) {
        return (bitmap[firstword] &
                ((~UINT64_C(0)) >> ((63 - lenminusone) % 64))
                    << (start % 64)) == 0;
    }
    if (((bitmap[firstword] & ((~UINT64_C(0)) << (start % 64)))) != 0)
        return false;
    for (uint32_t i = firstword + 1; i < endword; i++) {
        if (bitmap[i] != 0)
            return false;
 ...
 */
static inline void bitset_set_lenrange(uint64_t* bitmap,
                                       uint32_t start,
                                       uint32_t lenminusone) {
    uint32_t firstword = start / 64;
    uint32_t endword = (start + lenminusone) / 64;
 ...
 */
static inline void bitset_flip_range(uint64_t* bitmap,
                                     uint32_t start,
                                     uint32_t end) {
    if (start == end)
        return;
 ...
 */
static inline void bitset_reset_range(uint64_t* bitmap,
                                      uint32_t start,
                                      uint32_t end) {
    if (start == end)
        return;
 ...
 */
size_t bitset_extract_setbits_avx2(uint64_t* bitset,
                                   size_t length,
                                   void* vout,
                                   size_t outcapacity,
                                   uint32_t base);
 ...
 */
static inline void CSA(__m256i* h,
                       __m256i* l,
                       __m256i a,
                       __m256i b,
                       __m256i c) {
...
static inline bool array_container_contains_range(const array_container_t* arr,
    ...
    const int32_t end =
        advanceUntil(carr, start - 1, arr->cardinality, re_included);
...
static inline void array_container_add_range(array_container_t* array,
                                             ...
                                             uint32_t max) {
    int32_t nvals_greater =
        count_greater(array->array, array->cardinality, max);
    int32_t nvals_less =
        count_less(array->array, array->cardinality - nvals_greater, min);
...
static inline bool bitset_container_get_range(const bitset_container_t* bitset,
    ...
    if (start == end)
        return ((bitset->array[end] & first & last) == (first & last));
    if ((bitset->array[start] & first) != first)
        return false;

    if ((end < BITSET_CONTAINER_SIZE_IN_WORDS) &&
        ((bitset->array[end] & last) != last)) {
        return false;
    }

    for (uint16_t i = start + 1;
         (i < BITSET_CONTAINER_SIZE_IN_WORDS) && (i < end); ++i) {
        if (bitset->array[i] != UINT64_C(0xFFFFFFFFFFFFFFFF))
            return false;
    }
...
bool bitset_container_iterate(const bitset_container_t* cont,
                              uint32_t base,
                              roaring_iterator iterator,
                              void* ptr);
bool bitset_container_iterate64(const bitset_container_t* cont,
                                uint32_t base,
                                roaring_iterator64 iterator,
                                uint64_t high_bits,
                                void* ptr);
 ...
 */
static inline int32_t rle16_find_run(const rle16_t* array,
                                     int32_t lenarray,
                                     uint16_t ikey) {
    int32_t low = 0;
    int32_t high = lenarray - 1;
 ...
 */
static inline int32_t rle16_count_less(const rle16_t* array,
                                       int32_t lenarray,
                                       uint16_t key) {
    if (lenarray == 0)
        return 0;
 ...
 */
static inline void run_container_append(run_container_t* run,
                                        rle16_t vl,
                                        rle16_t* previousrl) {
 ...
 * The caller is responsible for allocating enough memory in out.
 */
int run_container_to_uint32_array(void* vout,
...
bool run_container_iterate(const run_container_t* cont,
                           uint32_t base,
                           roaring_iterator iterator,
                           void* ptr);
bool run_container_iterate64(const run_container_t* cont,
                             uint32_t base,
                             roaring_iterator64 iterator,
                             uint64_t high_bits,
                             void* ptr);
 ...
 */
int32_t run_container_read(int32_t cardinality,
...
static inline void run_container_remove_range(run_container_t* run,
    ...
    if (first >= 0 && min > run->runs[first].value &&
        ...
        run->runs[first + 1].length =
            (run->runs[first].value + run->runs[first].length) - (max + 1);
...
int run_run_container_ixor(run_container_t* src_1,
                           const run_container_t* src_2,
                           void** dst);
#endif
 ...
 **/
void* get_copy_of_container(void* container,
                            uint8_t* typecode,
                            bool copy_on_write);
...
inline const void* container_unwrap_shared(
    const void* candidate_shared_container,
    uint8_t* type) {
    if (*type == SHARED_CONTAINER_TYPE_CODE) {
        *type =
        ...
        return ((const shared_container_t*)candidate_shared_container)
            ->container;
    } else {
...
/* access to container underneath */
inline void* container_mutable_unwrap_shared(void* candidate_shared_container,
                                             uint8_t* type) {
    if (*type == SHARED_CONTAINER_TYPE_CODE) {
        *type = ((shared_container_t*)candidate_shared_container)->typecode;
        assert(*type != SHARED_CONTAINER_TYPE_CODE);
...
static inline void* get_writable_copy_if_shared(
    void* candidate_shared_container,
    uint8_t* type) {
    if (*type == SHARED_CONTAINER_TYPE_CODE) {
    ...
    switch (typecode) {
        ...
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_cardinality(
                (const array_container_t*)container);
        case RUN_CONTAINER_TYPE_CODE:
            return run_container_cardinality((const run_container_t*)container);
    }
    assert(false);
    __builtin_unreachable();
    return 0;  // unreached
...
static inline int container_shrink_to_fit(void* container, uint8_t typecode) {
    ...
    switch (typecode) {
        ...
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_shrink_to_fit((array_container_t*)container);
        case RUN_CONTAINER_TYPE_CODE:
...
static inline void* container_from_range(uint8_t* type,
                                         uint32_t min,
                                         uint32_t max,
                                         uint16_t step) {
    if (step == 0)
        return NULL;  // being paranoid
    if (step == 1) {
 ...
 *
 */
static inline int32_t container_write(const void* container,
                                      uint8_t typecode,
                                      char* buf) {
    container = container_unwrap_shared(container, &typecode);
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_write((const bitset_container_t*)container,
                                          buf);
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_write((const array_container_t*)container,
                                         buf);
        case RUN_CONTAINER_TYPE_CODE:
    ...
    switch (typecode) {
        ...
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_size_in_bytes(
                (const array_container_t*)container);
        case RUN_CONTAINER_TYPE_CODE:
            return run_container_size_in_bytes(
                (const run_container_t*)container);
    }
    assert(false);
    __builtin_unreachable();
 ...
 */
void container_printf_as_uint32_array(const void* container,
                                      uint8_t typecode,
                                      uint32_t base);
 ...
 * memory deallocation
 */
static inline void* container_add(void* container,
                                  uint16_t val,
                                  uint8_t typecode,
 ...
 * memory deallocation
 */
static inline void* container_remove(void* container,
                                     uint16_t val,
                                     uint8_t typecode,
                                     uint8_t* new_typecode) {
    container = get_writable_copy_if_shared(container, &typecode);
 ...
 */
inline bool container_contains(const void* container,
                               uint16_t val,
                               uint8_t typecode) {
    container = container_unwrap_shared(container, &typecode);
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_get((const bitset_container_t*)container,
                                        val);
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_contains((const array_container_t*)container,
                                            val);
        case RUN_CONTAINER_TYPE_CODE:
 ...
 */
static inline bool container_contains_range(const void* container,
                                            uint32_t range_start,
                                            uint32_t range_end,
                                            uint8_t typecode) {
    container = container_unwrap_shared(container, &typecode);
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_get_range(
                (const bitset_container_t*)container, range_start, range_end);
        case ARRAY_CONTAINER_TYPE_CODE:
            return array_container_contains_range(
                (const array_container_t*)container, range_start, range_end);
        case RUN_CONTAINER_TYPE_CODE:
            return run_container_contains_range(
                (const run_container_t*)container, range_start, range_end);
        default:
            assert(false);
            __builtin_unreachable();
            return false;
...
int32_t container_serialize(const void* container,
                            uint8_t typecode,
                            char* buf) WARN_UNUSED;
 ...
 */
static inline bool container_equals(const void* c1,
                                    uint8_t type1,
                                    const void* c2,
                                    uint8_t type2) {
 ...
 */
static inline bool container_is_subset(const void* c1,
                                       uint8_t type1,
                                       const void* c2,
                                       uint8_t type2) {
    c1 = container_unwrap_shared(c1, &type1);
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        ...
        case CONTAINER_PAIR(RUN_CONTAINER_TYPE_CODE,
                            BITSET_CONTAINER_TYPE_CODE):
            return run_container_is_subset_bitset(
        ...
        case CONTAINER_PAIR(ARRAY_CONTAINER_TYPE_CODE,
                            BITSET_CONTAINER_TYPE_CODE):
            return array_container_is_subset_bitset(
 ...
 */
static inline void* container_and(const void* c1,
                                  uint8_t type1,
                                  const void* c2,
                                  uint8_t type2,
                                  uint8_t* result_type) {
 ...
 */
static inline int container_and_cardinality(const void* c1,
                                            uint8_t type1,
                                            const void* c2,
                                            uint8_t type2) {
    c1 = container_unwrap_shared(c1, &type1);
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        case CONTAINER_PAIR(BITSET_CONTAINER_TYPE_CODE,
                            BITSET_CONTAINER_TYPE_CODE):
            return bitset_container_and_justcard((const bitset_container_t*)c1,
                                                 (const bitset_container_t*)c2);
        case CONTAINER_PAIR(ARRAY_CONTAINER_TYPE_CODE,
                            ARRAY_CONTAINER_TYPE_CODE):
 ...
 */
static inline bool container_intersect(const void* c1,
                                       uint8_t type1,
                                       const void* c2,
                                       uint8_t type2) {
    c1 = container_unwrap_shared(c1, &type1);
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        case CONTAINER_PAIR(BITSET_CONTAINER_TYPE_CODE,
                            BITSET_CONTAINER_TYPE_CODE):
            return bitset_container_intersect((const bitset_container_t*)c1,
                                              (const bitset_container_t*)c2);
        case CONTAINER_PAIR(ARRAY_CONTAINER_TYPE_CODE,
                            ARRAY_CONTAINER_TYPE_CODE):
        ...
        case CONTAINER_PAIR(BITSET_CONTAINER_TYPE_CODE,
                            ARRAY_CONTAINER_TYPE_CODE):
            return array_bitset_container_intersect(
                (const array_container_t*)c2, (const bitset_container_t*)c1);
        case CONTAINER_PAIR(ARRAY_CONTAINER_TYPE_CODE,
                            BITSET_CONTAINER_TYPE_CODE):
            return array_bitset_container_intersect(
...
*/
static inline void* container_iand(void* c1,
                                   uint8_t type1,
                                   const void* c2,
                                   uint8_t type2,
                                   uint8_t* result_type) {
 ...
 * is responsible for deallocation.
 */
static inline void* container_or(const void* c1,
                                 uint8_t type1,
                                 const void* c2,
                                 uint8_t type2,
                                 uint8_t* result_type) {
 ...
 */
static inline void* container_lazy_or(const void* c1,
                                      uint8_t type1,
                                      const void* c2,
                                      uint8_t type2,
                                      uint8_t* result_type) {
 ...
 * (and possibly new) container
 */
static inline void* container_ior(void* c1,
                                  uint8_t type1,
                                  const void* c2,
                                  uint8_t type2,
                                  uint8_t* result_type) {
 ...
 * cardinality. It requires repair later on the generated containers.
 */
static inline void* container_lazy_ior(void* c1,
                                       uint8_t type1,
                                       const void* c2,
                                       uint8_t type2,
                                       uint8_t* result_type) {
 ...
 */
static inline void* container_xor(const void* c1,
                                  uint8_t type1,
                                  const void* c2,
                                  uint8_t type2,
                                  uint8_t* result_type) {
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        ...
        case CONTAINER_PAIR(ARRAY_CONTAINER_TYPE_CODE,
                            ARRAY_CONTAINER_TYPE_CODE):
            *result_type =
                array_array_container_xor((const array_container_t*)c1,
                                          (const array_container_t*)c2, &result)
                    ? BITSET_CONTAINER_TYPE_CODE
                    : ARRAY_CONTAINER_TYPE_CODE;
            return result;
        ...
        case CONTAINER_PAIR(BITSET_CONTAINER_TYPE_CODE,
                            RUN_CONTAINER_TYPE_CODE):
            *result_type =
                run_bitset_container_xor((const run_container_t*)c2,
                                         (const bitset_container_t*)c1, &result)
                    ? BITSET_CONTAINER_TYPE_CODE
                    : ARRAY_CONTAINER_TYPE_CODE;
            return result;
        ...
        case CONTAINER_PAIR(RUN_CONTAINER_TYPE_CODE,
            ...
            *result_type =
                run_bitset_container_xor((const run_container_t*)c1,
                                         (const bitset_container_t*)c2, &result)
                    ? BITSET_CONTAINER_TYPE_CODE
                    : ARRAY_CONTAINER_TYPE_CODE;
            return result;
 ...
 * cardinality. It requires repair later on the generated containers.
 */
static inline void* container_lazy_xor(const void* c1,
                                       uint8_t type1,
                                       const void* c2,
                                       uint8_t type2,
                                       uint8_t* result_type) {
 ...
 * (and possibly new) container
 */
static inline void* container_ixor(void* c1,
                                   uint8_t type1,
                                   const void* c2,
                                   uint8_t type2,
                                   uint8_t* result_type) {
 ...
 * cardinality. It requires repair later on the generated containers.
 */
static inline void* container_lazy_ixor(void* c1,
                                        uint8_t type1,
                                        const void* c2,
                                        uint8_t type2,
                                        uint8_t* result_type) {
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        ...
        default:
            ...
            if ((type1 == BITSET_CONTAINER_TYPE_CODE) &&
                (((const bitset_container_t*)c1)->cardinality ==
                 BITSET_UNKNOWN_CARDINALITY)) {
                ((bitset_container_t*)c1)->cardinality =
                    bitset_container_compute_cardinality(
                        (bitset_container_t*)c1);
            }
 ...
 * memory, caller is responsible for deallocation.
 */
static inline void* container_andnot(const void* c1,
                                     uint8_t type1,
                                     const void* c2,
                                     uint8_t type2,
                                     uint8_t* result_type) {
    ...
    switch (CONTAINER_PAIR(type1, type2)) {
        ...
        case CONTAINER_PAIR(BITSET_CONTAINER_TYPE_CODE,
            ...
            *result_type =
                bitset_run_container_andnot((const bitset_container_t*)c1,
                                            (const run_container_t*)c2, &result)
                    ? BITSET_CONTAINER_TYPE_CODE
                    : ARRAY_CONTAINER_TYPE_CODE;
            return result;
 ...
 * (and possibly new) container
 */
static inline void* container_iandnot(void* c1,
                                      uint8_t type1,
                                      const void* c2,
                                      uint8_t type2,
                                      uint8_t* result_type) {
 ...
 */
static inline bool container_iterate(const void* container,
                                     uint8_t typecode,
                                     uint32_t base,
                                     roaring_iterator iterator,
                                     void* ptr) {
...
static inline bool container_iterate64(const void* container,
                                       uint8_t typecode,
                                       uint32_t base,
                                       roaring_iterator64 iterator,
                                       uint64_t high_bits,
    ...
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_iterate64(
                (const bitset_container_t*)container, base, iterator, high_bits,
                ptr);
        case ARRAY_CONTAINER_TYPE_CODE:
...
static inline void* container_not(const void* c,
                                  uint8_t typ,
                                  uint8_t* result_type) {
    ...
    switch (typ) {
        case BITSET_CONTAINER_TYPE_CODE:
            *result_type =
                bitset_container_negation((const bitset_container_t*)c, &result)
                    ? BITSET_CONTAINER_TYPE_CODE
                    : ARRAY_CONTAINER_TYPE_CODE;
            return result;
        case ARRAY_CONTAINER_TYPE_CODE:
...
static inline void* container_not_range(const void* c,
                                        uint8_t typ,
                                        uint32_t range_start,
                                        uint32_t range_end,
                                        uint8_t* result_type) {
...
static inline void* container_inot_range(void* c,
                                         uint8_t typ,
                                         uint32_t range_start,
                                         uint32_t range_end,
                                         uint8_t* result_type) {
 ...
 * Otherwise, it returns false and update start_rank.
 */
static inline bool container_select(const void* container,
                                    uint8_t typecode,
                                    uint32_t* start_rank,
                                    uint32_t rank,
                                    uint32_t* element) {
...
static inline uint16_t container_maximum(const void* container,
    ...
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_maximum(
                (const bitset_container_t*)container);
        case ARRAY_CONTAINER_TYPE_CODE:
...
static inline uint16_t container_minimum(const void* container,
    ...
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_minimum(
                (const bitset_container_t*)container);
        case ARRAY_CONTAINER_TYPE_CODE:
...
static inline int container_rank(const void* container,
                                 uint8_t typecode,
                                 uint16_t x) {
    container = container_unwrap_shared(container, &typecode);
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            return bitset_container_rank((const bitset_container_t*)container,
                                         x);
        case ARRAY_CONTAINER_TYPE_CODE:
 ...
 * (and possibly new) container.
 */
static inline void* container_add_range(void* container,
                                        uint8_t type,
                                        uint32_t min,
                                        uint32_t max,
                                        uint8_t* result_type) {
    ...
    switch (type) {
        ...
        case ARRAY_CONTAINER_TYPE_CODE: {
            ...
            int32_t nvals_greater =
                count_greater(array->array, array->cardinality, max);
            int32_t nvals_less = count_less(
                array->array, array->cardinality - nvals_greater, min);
        ...
        case RUN_CONTAINER_TYPE_CODE: {
            ...
            int32_t nruns_greater =
                rle16_count_greater(run->runs, run->n_runs, max);
            int32_t nruns_less =
                rle16_count_less(run->runs, run->n_runs - nruns_greater, min);
 ...
 * container.
 */
static inline void* container_remove_range(void* container,
                                           uint8_t type,
                                           uint32_t min,
                                           uint32_t max,
                                           uint8_t* result_type) {
 ...
 */
bool ra_copy(const roaring_array_t* source,
 ...
 */
bool ra_overwrite(const roaring_array_t* source,
                  roaring_array_t* dest,
                  bool copy_on_write);
 ...
 */
inline void* ra_get_container_at_index(const roaring_array_t* ra,
                                       uint16_t i,
                                       uint8_t* typecode) {
 ...
 */
void ra_insert_new_key_value_at(roaring_array_t* ra,
                                int32_t i,
                                uint16_t key,
                                void* container,
                                uint8_t typecode);
 ...
 * at index index
 */
void ra_append_copy(roaring_array_t* ra,
                    const roaring_array_t* sa,
                    uint16_t index,
                    bool copy_on_write);
 ...
 */
void ra_append_copy_range(roaring_array_t* ra,
                          const roaring_array_t* sa,
                          int32_t start_index,
                          int32_t end_index,
                          bool copy_on_write);
 ...
 */
void ra_append_copies_until(roaring_array_t* ra,
                            const roaring_array_t* sa,
...
void ra_append_copies_after(roaring_array_t* ra,
                            const roaring_array_t* sa,
 ...
 **/
void ra_append_move_range(roaring_array_t* ra,
                          roaring_array_t* sa,
                          int32_t start_index,
                          int32_t end_index);
/**
 ...
 * [start_index, end_index)
 */
void ra_append_range(roaring_array_t* ra,
                     roaring_array_t* sa,
                     int32_t start_index,
                     int32_t end_index,
                     bool copy_on_write);
 ...
 * typecode.
 */
inline void ra_set_container_at_index(const roaring_array_t* ra,
                                      int32_t i,
                                      void* c,
                                      uint8_t typecode) {
    assert(i < ra->size);
...
inline int32_t ra_get_size(const roaring_array_t* ra) {
    return ra->size;
}

static inline int32_t ra_advance_until(const roaring_array_t* ra,
                                       uint16_t x,
                                       int32_t pos) {
...
bool ra_range_uint32_array(const roaring_array_t* ra,
                           size_t offset,
                           size_t limit,
                           uint32_t* ans);
 ...
 */
bool ra_portable_deserialize(roaring_array_t* ra,
                             const char* buf,
...
//
void ra_copy_range(roaring_array_t* ra,
                   uint32_t begin,
                   uint32_t end,
                   uint32_t new_begin);
 ...
 * Add all values in range [min, max]
 */
void roaring_bitmap_add_range_closed(roaring_bitmap_t* ra,
                                     uint32_t min,
                                     uint32_t max);
 ...
 * Add all values in range [min, max)
 */
inline void roaring_bitmap_add_range(roaring_bitmap_t* ra,
                                     uint64_t min,
                                     uint64_t max) {
    if (max == min)
        return;
...
void roaring_bitmap_remove_range_closed(roaring_bitmap_t* ra,
                                        uint32_t min,
                                        uint32_t max);

/** Remove all values in range [min, max) */
inline void roaring_bitmap_remove_range(roaring_bitmap_t* ra,
                                        uint64_t min,
                                        uint64_t max) {
    if (max == min)
        return;
...
/**
 * Convert the bitmap to an array from "offset" by "limit". Write the output to
 * "ans". so, you can get data in paging. caller is responsible to ensure that
 * there is enough memory allocated (e.g., ans =
 * malloc(roaring_bitmap_get_cardinality(limit)
 *   * sizeof(uint32_t))
 * Return false in case of failure (e.g., insufficient memory)
 */
bool roaring_bitmap_range_uint32_array(const roaring_bitmap_t* ra,
                                       size_t offset,
                                       size_t limit,
                                       uint32_t* ans);
 ...
 * buffer overflow. For a safer approach, call
 * roaring_bitmap_portable_deserialize_safe.
 */
roaring_bitmap_t* roaring_bitmap_portable_deserialize(const char* buf);

/**
 * read a bitmap from a serialized version in a safe manner (reading up to
 * maxbytes). This is meant to be compatible with the Java and Go versions. See
 * format specification at https://github.com/RoaringBitmap/RoaringFormatSpec In
 * case of failure, a null pointer is returned.
 */
 ...
 * Buffer must contain data previously written by
 * roaring_bitmap_frozen_serialize(), and additionally its beginning must be
 * aligned by 32 bytes. Length must be equal exactly to
 ...
 */
bool roaring_iterate(const roaring_bitmap_t* ra,
                     roaring_iterator iterator,
                     void* ptr);

bool roaring_iterate64(const roaring_bitmap_t* ra,
                       roaring_iterator64 iterator,
                       uint64_t high_bits,
                       void* ptr);
...
void roaring_bitmap_flip_inplace(roaring_bitmap_t* x1,
                                 uint64_t range_start,
                                 uint64_t range_end);
 ...
 */
bool roaring_bitmap_select(const roaring_bitmap_t* ra,
                           uint32_t rank,
                           uint32_t* element);
/**
...
/**
 * Move the iterator to the first value >= val. If there is a such a value, then
 * it->has_value is true. The new value is in it->current_value. For
 * convenience, returns it->has_value.
 */
