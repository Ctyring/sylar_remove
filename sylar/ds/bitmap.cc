...
// static uint8_t HEAD[] = {0x00, 0x80, 0xC0, 0xE0, 0xF0, 0xF8, 0xFC, 0xFE,
// 0xFF}; static uint8_t TAIL[] = {0x00, 0x01, 0x03, 0x07, 0x0F, 0x1F, 0x3F,
// 0x7F, 0xFF}; static uint8_t POS[] = {0x01, 0x02, 0x4, 0x8, 0x10, 0x20, 0x40,
// 0x80};
        ...
        for (uint32_t i = 0; i < b.m_dataSize; ++i) {
            ...
            if (cur & COMPRESS_MASK) {
                ...
                for (uint32_t n = i + 1; n < b.m_dataSize; ++n) {
                    if ((b.m_data[n] & COMPRESS_MASK) &&
                        (v == (bool)(b.m_data[n] & VALUE_MASK))) {
                        count = (((uint32_t)(b.m_data[n] & COUNT_MASK))
                                 << (VALUE_SIZE * (n - tmp_i))) |
                                count;
                        ++i;
                    } else {
...
bool Bitmap::operator==(const Bitmap& b) const {
    ...
    if (m_compress != b.m_compress || m_size != b.m_size ||
        m_dataSize != b.m_dataSize) {
        return false;
    }
...
std::string Bitmap::toString() const {
    std::stringstream ss;
    ss << "[Bitmap compress=" << m_compress << " size=" << m_size
       << " data_size=" << m_dataSize << " data=";
...
Bitmap::ptr Bitmap::compress() const {
    ...
    for (uint32_t i = 0; i < m_dataSize; ++i) {
        ...
        if ((cur_count / VALUE_SIZE) > 1) {
            ...
            while (cur_count) {
                data[dst_cur_pos++] =
                    cur_value ? (COMPRESS_MASK | VALUE_MASK |
                                 (cur_count & COUNT_MASK))
    ...
    for (uint32_t i = 0; i < m_dataSize;) {
        ...
        if (cur & COMPRESS_MASK) {
            ...
            for (uint32_t n = i + 1; n < m_dataSize; ++n) {
                if ((m_data[n] & COMPRESS_MASK) &&
                    (v == (bool)(m_data[n] & VALUE_MASK))) {
                    count = (((uint32_t)(m_data[n] & COUNT_MASK))
                             << (VALUE_SIZE * (n - tmp_i))) |
                            count;
                    ++i;
                } else {
...
bool Bitmap::any() const {
    ...
    if (!m_compress) {
        ...
        return m_data[m_dataSize - 1] &
               MASK[m_size % VALUE_SIZE];  //(((base_type)1 << (m_size %
                                           //VALUE_SIZE)) - 1);
    } else {
        for (uint32_t i = 0; i < m_dataSize; ++i) {
...
void Bitmap::resize(uint32_t size, bool v) {
    ...
    if (len > m_dataSize) {
        ...
        if (v) {
            memset(new_data + m_dataSize, 0xFF,
                   (len - m_dataSize) * sizeof(base_type));
        } else {
            memset(new_data + m_dataSize, 0,
                   (len - m_dataSize) * sizeof(base_type));
        }
        ...
        uint32_t left = m_size % VALUE_SIZE;
        if (v) {
            new_data[m_dataSize - 1] |=
                MASK[left];  //((base_type)1 << left) - 1;
        } else {
            new_data[m_dataSize - 1] &=
                MASK[left];  //((base_type)1 << left) - 1;
        }
...
void Bitmap::foreach (std::function<bool(uint32_t)> cb) {
    ...
    for (uint32_t i = 0; i < max_size; ++i) {
        ...
        if (!tmp) {
            ...
            for (uint32_t n = 0; n < U64_DIV_BASE; ++n) {
                ...
                for (uint32_t x = 0; x < VALUE_SIZE && cur_pos < m_size;
                     ++x, ++cur_pos) {
    ...
    for (uint32_t i = max_size * U64_DIV_BASE; i < m_dataSize; ++i) {
        ...
        if (!tmp) {
            ...
            for (uint32_t n = 0; n < VALUE_SIZE && cur_pos < m_size;
                 ++n, ++cur_pos) {
                if (tmp & POS[n]) {  //(1UL << n)) {
                    if (!cb(cur_pos)) {
                        return;
    ...
    if (m_size % U64_VALUE_SIZE) {
        ...
        for (int32_t i = start; i >= (int32_t)(max_size * U64_DIV_BASE); --i) {
            ...
            if (!tmp) {
                ...
                if (i == start) {
                    if (m_size % VALUE_SIZE == 0) {
                        for (int32_t n = VALUE_SIZE - 1; n >= 0;
                             --n, --cur_pos) {
                        ...
                        cur_pos += 1;
                        for (int32_t n = m_size % VALUE_SIZE; n >= 0;
                             --n, --cur_pos) {
                            if (tmp & POS[n]) {  //((base_type)1 << n)) {
                                if (!cb(cur_pos)) {
                                    return;
...
void Bitmap::iterator::next() {
    ...
    if (left) {
        v = m_data[pos];
        for (int32_t i = left; i < (int32_t)VALUE_SIZE && m_pos < m_size;
             ++i, ++m_pos) {
    ...
    for (int32_t i = m_pos / VALUE_SIZE; i < m_dataSize && m_pos < m_size;
         ++i) {
        v = m_data[i];
        if (!v) {
            m_pos += VALUE_SIZE;
        ...
        for (int32_t n = 0; n < (int32_t)VALUE_SIZE && m_pos < m_size;
             ++n, ++m_pos) {
...
uint32_t Bitmap::getCount() const {
    if (!m_compress) {
        uint32_t len =
            m_dataSize / U64_DIV_BASE;  // - (m_dataSize % 8 == 0 ? 1 : 0);
        uint32_t count = 0;
        uint32_t cur_pos = 0;
