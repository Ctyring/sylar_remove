...
class Dict {
   ...
   public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        auto it =
            BinarySearch(m_datas[pos].begin(), m_datas[pos].end(), Node(k));
        // SYLAR_ASSERT(it == std::find(m_datas[pos].begin(),
        // m_datas[pos].end(), Node(k)));
    ...
    std::ostream& dump(std::ostream& os) {
        ...
        os << "[Dict total=" << m_total << " bucket=" << m_size
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if (speed == (uint64_t)-1) {
            ...
            sylar::WriteFixToStreamWithSpeed(os, (const char*)&vs[0], size,
                                             speed);
        }
        ...
        if (speed == (uint64_t)-1) {
            ...
            sylar::WriteFixToStreamWithSpeed(os, (const char*)&ns[0], size,
                                             speed);
        }
        ...
        do {
            try {
                ...
                if (speed == (uint64_t)-1) {
                    ...
                    if (!ReadFixFromStreamWithSpeed(is, (char*)&vs[0], size,
                                                    speed)) {
                        // LOG_ERROR() << "error";
                        break;
                    }
                ...
                if (speed == (uint64_t)-1) {
                    ...
                    if (!ReadFixFromStreamWithSpeed(is, (char*)&ns[0], size,
                                                    speed)) {
                        break;
                    }
                }
                // LOG_INFO() << size << ": " << (size / sizeof(Node)) << " : "
                // << ns.size();
...
class StringDict {
   public:
    ...
    std::ostream& dump(std::ostream& os) { return m_dict.dump(os); }

    void foreach (
        std::function<bool(const uint64_t& k, const char* v, size_t size)> cb) {
        m_dict.foreach (cb);
    }
