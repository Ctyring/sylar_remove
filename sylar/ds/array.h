...
class Array {
   public:
    ...
    T& at(uint64_t idx) { return m_data[idx]; }

    const T& at(uint64_t idx) const { return m_data[idx]; }

    T& operator[](uint64_t idx) { return m_data[idx]; }

    const T& operator[](uint64_t idx) const { return m_data[idx]; }

    void set(uint64_t idx, const T& v) { m_data[idx] = v; }

    const T& get(uint64_t idx) { return m_data[idx]; }
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if (speed == (uint64_t)-1) {
            ...
            WriteFixToStreamWithSpeed(os, (const char*)m_data,
                                      sizeof(T) * m_size, speed);
        }
        return (bool)os;
    }
    ...
    bool readFrom(std::istream& is, uint64_t speed = -1) {
        do {
            try {
                ...
                m_data = (T*)realloc(m_data, m_size * sizeof(T));
                if (speed == (uint64_t)-1) {
                    if (!ReadFixFromStream(is, (char*)m_data,
                                           m_size * sizeof(T))) {
                        break;
                    }
                } else {
                    if (!ReadFixFromStreamWithSpeed(
                            is, (char*)m_data, m_size * sizeof(T), speed)) {
                        break;
                    }
                }
            } catch (...) {
