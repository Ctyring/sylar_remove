...
class Bitmap {
   ...
   private:
    ...
    static const base_type COMPRESS_MASK =
        ((base_type)1 << (sizeof(base_type) * 8 - 1));
    static const base_type VALUE_MASK =
        ((base_type)1 << (sizeof(base_type) * 8 - 2));
    static const base_type COUNT_MASK =
        ((base_type)1 << (sizeof(base_type) * 8 - 2)) - 1;
    static const base_type NOT_VALUE_MASK = ~VALUE_MASK;
    static const base_type U64_DIV_BASE =
        (sizeof(uint64_t) / sizeof(base_type));
