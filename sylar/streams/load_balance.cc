...
std::string HolderStats::toString() {
    std::stringstream ss;
    ss << "[Stat total=" << m_total << " used_time=" << m_usedTime
       << " doing=" << m_doing << " timeouts=" << m_timeouts << " oks=" << m_oks
       << " errs=" << m_errs
...
void LoadBalanceItem::close() {
    if (m_stream) {
        ...
        sylar::WorkerMgr::GetInstance()->schedule(
            "service_io", [stream]() { stream->close(); });
    }
}
...
std::string LoadBalance::statusString(const std::string& prefix) {
    ...
    lock.unlock();
    std::stringstream ss;
    ss << prefix << "init_time: " << sylar::Time2Str(m_lastInitTime / 1000)
       << std::endl;
    for (auto& i : datas) {
...
float HolderStats::getWeight(float rate) {
    ...
    return std::min((m_oks * 1.0 / (m_usedTime + 1)) * 2.0, 50.0) *
           (1 - 4.0 * m_timeouts / base) * (1 - 1 * m_doing / base) *
...
LoadBalance::ptr SDLoadBalance::get(const std::string& domain,
...
ILoadBalance::Type SDLoadBalance::getType(const std::string& domain,
                                          const std::string& service) {
    RWMutexType::ReadLock lock(m_mutex);
...
LoadBalanceItem::ptr SDLoadBalance::createLoadBalanceItem(
    ILoadBalance::Type type) {
...
void SDLoadBalance::onServiceChange(
    ...
    SYLAR_LOG_INFO(g_logger)
        << "onServiceChange domain=" << domain << " service=" << service;
    auto type = getType(domain, service);
    ...
    for (auto& i : add_values) {
        ...
        if (!stream) {
            SYLAR_LOG_ERROR(g_logger)
                << "create stream fail, " << i.second->toString();
            continue;
        }
...
void SDLoadBalance::start() {
    m_sd->setServiceCallback(std::bind(
        &SDLoadBalance::onServiceChange, this, std::placeholders::_1,
