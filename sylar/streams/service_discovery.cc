...
void IServiceDiscovery::queryServer(const std::string& domain,
                                    const std::string& service) {
    sylar::RWMutex::WriteLock lock(m_mutex);
        ...
        if (pos == 0 || existsOrCreate(path.substr(0, pos))) {
            ...
            if (v != ZOK) {
                SYLAR_LOG_ERROR(g_logger)
                    << "create path=" << path << " error:" << zerror(v) << " ("
                    << v << ")";
                return false;
            }
            return true;
...
static std::string GetProvidersPath(const std::string& domain,
                                    const std::string& service) {
    return "/sylar/" + domain + "/" + service + "/providers";
}

static std::string GetConsumersPath(const std::string& domain,
                                    const std::string& service) {
...
bool ParseDomainService(const std::string& path,
                        std::string& domain,
                        std::string& service) {
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain,
    ...
    std::string new_val(1024, 0);
    int32_t rt =
        m_client->create(path + "/" + ip_and_port, data, new_val,
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain,
    ...
    if (service != "all") {
        ...
        std::string new_val(1024, 0);
        int32_t rt = m_client->create(path + "/" + m_selfInfo, m_selfData,
                                      new_val, &ZOO_OPEN_ACL_UNSAFE,
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    if (!ParseDomainService(path, domain, service)) {
        SYLAR_LOG_ERROR(g_logger)
            << "get_children path=" << path << " invalid path";
        return false;
    }
    {
        sylar::RWMutex::ReadLock lock(m_mutex);
        ...
        if (it->second.count(service) == 0 && it->second.count("all") == 0) {
            SYLAR_LOG_ERROR(g_logger)
                << "get_children path=" << path << " service=" << service
                << " not exists "
    ...
    if (v != ZOK) {
        SYLAR_LOG_ERROR(g_logger)
            << "get_children path=" << path << " fail, error:" << zerror(v)
            << " (" << v << ")";
        return false;
    }
    std::unordered_map<uint64_t, ServiceItemInfo::ptr> infos;
...
void ZKServiceDiscovery::onZKChanged(const std::string& path,
                                     ZKClient::ptr client) {
    SYLAR_LOG_INFO(g_logger) << "onZKChanged path=" << path;
}

void ZKServiceDiscovery::onZKDeleted(const std::string& path,
                                     ZKClient::ptr client) {
    SYLAR_LOG_INFO(g_logger) << "onZKDeleted path=" << path;
}

void ZKServiceDiscovery::onZKExpiredSession(const std::string& path,
                                            ZKClient::ptr client) {
    SYLAR_LOG_INFO(g_logger) << "onZKExpiredSession path=" << path;
    client->reconnect();
}

void ZKServiceDiscovery::onWatch(int type,
                                 int stat,
    ...
    SYLAR_LOG_ERROR(g_logger)
        << "onWatch hosts=" << m_hosts << " type=" << type << " stat=" << stat
