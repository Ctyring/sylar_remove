...
bool Table::init(const tinyxml2::XMLElement& node) {
    ...
    if (!cols) {
        SYLAR_LOG_ERROR(g_logger)
            << "table name=" << m_name << " columns is null";
        return false;
    }
    ...
    if (!col) {
        SYLAR_LOG_ERROR(g_logger)
            << "table name=" << m_name << " column is null";
        return false;
    }
    ...
    do {
        ...
        if (!col_ptr->init(*col)) {
            SYLAR_LOG_ERROR(g_logger)
                << "table name=" << m_name << " init column error";
            return false;
        }
    ...
    if (!idxs) {
        SYLAR_LOG_ERROR(g_logger)
            << "table name=" << m_name << " indexs is null";
        return false;
    }
    ...
    do {
        ...
        if (!idx_ptr->init(*idx)) {
            SYLAR_LOG_ERROR(g_logger)
                << "table name=" << m_name << " index init error";
            return false;
        }
        if (idx_names.insert(idx_ptr->getName()).second == false) {
            SYLAR_LOG_ERROR(g_logger)
                << "table name=" << m_name
                << " index name=" << idx_ptr->getName() << " exists";
            return false;
        }
        ...
        if (idx_ptr->isPK()) {
            if (has_pk) {
                SYLAR_LOG_ERROR(g_logger)
                    << "table name=" << m_name << " more than one pk";
                return false;
            }
            has_pk = true;
        }
        ...
        for (auto& x : cnames) {
            if (col_names.count(x) == 0) {
                SYLAR_LOG_ERROR(g_logger)
                    << "table name=" << m_name << " idx=" << idx_ptr->getName()
                    << " col=" << x << " not exists";
                return false;
            }
        }
...
void Table::gen_inc(const std::string& path) {
    ...
    ofs << "#ifndef " << GetAsDefineMacro(m_namespace + class_name + ".h")
        << std::endl;
    ofs << "#define " << GetAsDefineMacro(m_namespace + class_name + ".h")
        << std::endl;
    ...
    std::set<std::string> incs = {"sylar/db/sqlite3.h", "sylar/util.h"};
    for (auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    ofs << "friend class " << GetAsClassName(class_name_dao) << ";"
        << std::endl;
    ofs << "public:" << std::endl;
    ofs << "    typedef std::shared_ptr<" << GetAsClassName(class_name)
        << "> ptr;" << std::endl;
    ofs << std::endl;
    ...
    std::sort(cols.begin(), cols.end(),
              [](const Column::ptr& a, const Column::ptr& b) {
    ...
    ofs << "#endif //" << GetAsDefineMacro(m_namespace + class_name + ".h")
        << std::endl;
}
...
std::string Table::genToStringSrc(const std::string& class_name) {
    std::stringstream ss;
    ss << "std::string " << GetAsClassName(class_name)
       << "::toJsonString() const {" << std::endl;
...
void Table::gen_dao_inc(std::ofstream& ofs) {
    ...
    ofs << "class " << GetAsClassName(class_name_dao) << " {" << std::endl;
    ofs << "public:" << std::endl;
    ofs << "    typedef std::shared_ptr<" << GetAsClassName(class_name_dao)
        << "> ptr;" << std::endl;
    ofs << "    static int Update(" << GetAsClassName(class_name)
        << "::ptr info, " << m_dbclass << "::ptr conn);" << std::endl;
    ...
    for (auto& i : m_idxs) {
        if (i->getDType() == Index::TYPE_UNIQ) {
            ...
            ofs << GetAsClassName(tmp) << "(";
            ofs << "std::vector<" << GetAsClassName(class_name)
                << "::ptr>& results, ";
            for (auto& c : i->getCols()) {
                auto d = getCol(c);
                ofs << "const " << d->getDTypeString() << "& "
    ...
    ofs << "    static int CreateTable(" << m_dbclass << "::ptr info);"
        << std::endl;
    ofs << "};" << std::endl;
}
...
void Table::gen_dao_src(std::ofstream& ofs) {
    std::string class_name = m_name + m_subfix;
    std::string class_name_dao = class_name + "_dao";
    ofs << "int " << GetAsClassName(class_name_dao) << "::Update("
        << GetAsClassName(class_name) << "::ptr info, " << m_dbclass
        << "::ptr conn) {" << std::endl;
    ofs << "    return conn->execStmt(\"update " << m_name << " set";
    auto pks = getPKs();
    bool is_first = true;
    for (auto& i : m_cols) {
        if (is_exists(pks, i)) {
    ...
    ofs << "int " << GetAsClassName(class_name_dao) << "::Insert("
        << GetAsClassName(class_name) << "::ptr info, " << m_dbclass
        << "::ptr conn) {" << std::endl;
    ...
    ofs << "    return conn->execStmt(\"delete from " << m_name << " where";
    is_first = true;
    for (auto& i : pks) {
        if (!is_first) {
            ofs << " and";
    ...
    for (auto& i : m_idxs) {
        if (i->getDType() == Index::TYPE_UNIQ ||
            ...
            ofs << m_dbclass << "::ptr conn) {" << std::endl;
            ofs << "    return conn->execStmt(\"delete from " << m_name
                << " where";
            is_first = true;
    ...
    ofs << "int " << GetAsClassName(class_name_dao) << "::QueryAll(std::vector<"
        << GetAsClassName(class_name) << "::ptr>& results, " << m_dbclass
        << "::ptr conn) {" << std::endl;
    ofs << "    auto rt = conn->queryStmt(\"select ";
    is_first = true;
    for (auto& i : m_cols) {
        if (!is_first) {
            ofs << ", ";
    ...
    for (size_t i = 0; i < m_cols.size(); ++i) {
        ofs << "        v->" << GetAsMemberName(m_cols[i]->getName())
            << " = rt->get";
    ...
    ofs << "int " << GetAsClassName(class_name_dao) << "::CreateTable("
