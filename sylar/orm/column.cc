...
bool Column::init(const tinyxml2::XMLElement& node) {
    ...
    if (!node.Attribute("type")) {
        SYLAR_LOG_ERROR(g_logger)
            << "column name=" << m_name << " type is null";
        return false;
    }
...
std::string Column::getSetFunImpl(const std::string& class_name,
                                  int idx) const {
    std::stringstream ss;
    ss << "void " << GetAsClassName(class_name)
       << "::" << GetAsSetFunName(m_name) << "(const " << TypeToString(m_dtype)
       << "& v) {" << std::endl;
