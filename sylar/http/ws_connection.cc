...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(
    ...
    if (!uri) {
        return std::make_pair(
            std::make_shared<HttpResult>((int)HttpResult::Error::INVALID_URL,
...
std::pair<HttpResult::ptr, WSConnection::ptr> WSConnection::Create(
    ...
    if (rsp->getStatus() != HttpStatus::SWITCHING_PROTOCOLS) {
        return std::make_pair(
            std::make_shared<HttpResult>(
                50, rsp, "not websocket server " + addr->toString()),
            nullptr);
    }
    return std::make_pair(
        std::make_shared<HttpResult>((int)HttpResult::Error::OK, rsp, "ok"),
        conn);
}
...
int32_t WSConnection::sendMessage(const std::string& msg,
                                  int32_t opcode,
                                  bool fin) {
