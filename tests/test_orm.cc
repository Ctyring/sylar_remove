...
int main(int argc, char** argv) {
    ...
    if (argc == 1) {
        ...
        db = m;
        std::cout << "create table: "
                  << test::orm::UserInfoDao::CreateTableMySQL(db) << std::endl;
    }
    for (int i = 0; i < 10; ++i) {
        ...
        u->setStatus(i % 2);
        std::cout << "i= " << i << " - "
                  << test::orm::UserInfoDao::Insert(u, db);
        std::cout << " - " << u->toJsonString() << std::endl;
    }
    std::vector<test::orm::UserInfo::ptr> us;
    std::cout << "query_by_status: "
              << test::orm::UserInfoDao::QueryByStatus(us, 1, db) << std::endl;
    ...
    std::cout << "delete: " << test::orm::UserInfoDao::DeleteByStatus(1, db)
              << std::endl;
    us.clear();
    std::cout << "query_by_status: "
              << test::orm::UserInfoDao::QueryByStatus(us, 0, db) << std::endl;
    for (size_t i = 0; i < us.size(); ++i) {
        std::cout << "i=" << i << " - " << us[i]->toJsonString() << std::endl;
    }
    us.clear();
    std::cout << "query_all: " << test::orm::UserInfoDao::QueryAll(us, db)
              << std::endl;
    for (size_t i = 0; i < us.size(); ++i) {
        std::cout << "i=" << i << " - " << us[i]->toJsonString() << std::endl;
    }
