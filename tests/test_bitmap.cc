...
void init(uint32_t size, std::set<uint32_t> v = {}) {
    ...
    *c &= *b;
    ...
    for (auto it = b->begin_new(); !*it; it->next()) {
        v1.push_back(**it);
    }

    for (auto it = b->rbegin_new(); !*it; it->next()) {
        v2.push_back(**it);
    }
...
void test_compress(size_t size, std::set<uint32_t> v = {}) {
    ...
    if (*b != *bb) {
        ...
        SYLAR_LOG_INFO(g_logger)
            << "size=" << size << " - " << sylar::Join(v.begin(), v.end(), ",");
...
void test_op(size_t size, std::set<uint32_t> v1, std::set<uint32_t> v2) {
    ...
    std::set_union(v1.begin(), v1.end(), v2.begin(), v2.end(),
                   std::back_inserter(or_sv));
    std::set_intersection(v1.begin(), v1.end(), v2.begin(), v2.end(),
                          std::back_inserter(and_sv));
...
int main(int argc, char** argv) {
    ...
    // init(64,
    // {4,5,7,8,14,17,18,19,20,21,23,27,29,32,35,39,40,41,43,46,48,50,51,53,62});
    ...
    // init(41,
    // {0,7,8,10,12,16,18,22,23,24,27,29,31,32,34,35,37,41,42,46,48,51,53,54,58,60,61,63,64,67,71});
    // init(60,
    // {2,7,10,11,12,14,15,16,18,19,20,21,22,23,28,29,30,34,38,40,41,45,49,50,54,55});
    init(72, {2,  7,  9,  10, 13, 14, 17, 19, 20, 22, 28, 31, 32, 35, 36,
              39, 41, 44, 45, 47, 51, 54, 59, 60, 61, 62, 64, 67, 69});
    test_compress(41, {36});
